...
class MapListViewPage extends StatefulWidget {
  const MapListViewPage({Key key}) : super(key: key);

  @override
  MapListViewPageState createState() => MapListViewPageState();
}

class MapListViewPageState extends State<MapListViewPage> {
  ...
  Future<void> loadMoreData() async {
    ...
    var loadDigitalFiles = await natSearcher.searchMaps();
    if (mounted) {
      setState(() {
  ...
  void initState() {
    ...
    super.initState();
    initializeSearcher().then((value) => {showlistMaps()});
    //Add handle.
  ...
  Future<void> initializeSearcher() async {
    ...
    digitalFiles = await natSearcher.searchMaps();
    quantityOfMap = await natSearcher.countMap();
  }
              ...
              child: Padding(
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    SizedBox(height: 20),
                    Text(
                      "รายการค้นหาทั้งหมด ${quantityOfMap} รายการ",
  ...
  Widget _documentSectionCard(DigitalFile digitalFile) => Container(
        ...
        child: ListTile(
          ...
          subtitle: Padding(
            padding: const EdgeInsets.only(top: 10),
            child: Text(digitalFile.content.unitName),
          ),
        ),
      );
}
